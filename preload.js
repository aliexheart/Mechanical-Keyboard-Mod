...
// Bridges global key events from the main process into the page's
// window object, where thockboard.html's existing JS can pick them up.

const { contextBridge, ipcRenderer } = require('electron');

// uiohook keycodes → browser-compatible e.code strings
// Reference: https://github.com/kwhat/libuiohook/blob/main/include/uiohook.h
const KEYCODE_TO_CODE = {
  // Row 0 — numbers / function
  1:   'Escape',
  ...
  // Navigation
  3655: 'Home', 3663: 'End',
  3657: 'PageUp', 3665: 'PageDown',
  3666: 'Insert', 3637: 'Delete',
  // Arrows
  57416: 'ArrowUp',   57424: 'ArrowDown',
  57419: 'ArrowLeft', 57421: 'ArrowRight',
};

contextBridge.exposeInMainWorld('electronBridge', {
  onGlobalKeydown: (cb) => {
    ipcRenderer.on('global-keydown', (_event, data) => {
      const code = KEYCODE_TO_CODE[data.keycode] || 'KeyA';
      cb(code);
    });
  },
  onGlobalKeyup: (cb) => {
    ipcRenderer.on('global-keyup', (_event, data) => {
      const code = KEYCODE_TO_CODE[data.keycode] || 'KeyA';
      cb(code);
    });
  },
